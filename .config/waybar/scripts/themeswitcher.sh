...
choice=$(echo -e "$listNames" | rofi -dmenu -replace -i -config ~/.config/rofi/config-themes.rasi -no-show-icons -p "Themes" -format i)
IFS="~"
input=$listNames2
read -ra array <<<"$input"
...
if [ "$choice" ]; then
  ...
  echo "${listThemes[$choice + 1]}" >~/.cache/.themestyle
